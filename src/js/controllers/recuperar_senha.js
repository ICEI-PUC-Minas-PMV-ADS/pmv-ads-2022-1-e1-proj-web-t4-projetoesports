...
export class RecuperarSenhaController extends Controller
{
  constructor()
  {
    super();
  ...
  actions()
  {
    return {
      ...
      onRecoverPassword: function () {
        ...
        const recover_email = document.getElementById('recover_email');
        if (recover_email.value.trim().length === 0 || !validateEmail(recover_email.value)) {
          alert("Você precisa digitar um email valido");
          return;
        }
        const user = this.userRepository.getAll().find((u) => u.email === recover_email.value);
        if (!user) {
          alert("Seu email não foi localizado em nossa base de dados!");
          return;
        }
        alert("Você recebera um email com as instruções para recuperar sua senha")
      },
